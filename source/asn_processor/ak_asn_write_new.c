...
/* ----------------------------------------------------------------------------------------------- */
/*! @param tag_number номер тег
    @param p_data указатель на закодированные данные, тип которых соответствует номеру тега
    @param p_name имя данных, добавляемое по желанию (при отсутствие имени передается NULL)
...
/* ----------------------------------------------------------------------------------------------- */
/*! @param str IA5 строка
    @param pp_buff указатель на область памяти, в которую записывается результат кодирования
    @param p_size указатель на переменную, в которую запишется размер закодированных данных
    @return В случае успеха функция возввращает ak_error_ok (ноль).
    В противном случае, возвращается код ошибки.                                                   */
/* ----------------------------------------------------------------------------------------------- */
